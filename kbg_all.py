...
# iM 뱅크 기업금융 데이터
df = pd.read_csv(
    r'G:\내 드라이브\Colab Notebooks\iM\Team_project\훈련생 전달용\(아이엠뱅크) 2025 교육용 데이터\(아이엠뱅크) 2025 교육용 데이터.csv',
    encoding='cp949'
)
...
# 거래금액 (열 순서 의존)
if '창구거래금액' in df.columns and '요구불출금금액' in df.columns:
    df['거래금액'] = df.loc[:, '창구거래금액':'요구불출금금액'].sum(axis=1)
else:
    df['거래금액'] = 0.0
...
# ================================================================
# 6) 공통 가중평균 하이퍼파라미터 (전 업종 공통 적용)
# ================================================================
COMMON_FEATURES = [
    '법인_고객등급', '예금잔액', '투자잔액', '여신대출잔액',
    '대출비중', '신용카드사용금액', '거래금액'
]
...
weights_aligned = {k: v for k, v in WEIGHTED_OMEGA.items() if k in available_features}

if len(available_features) == 0:
    raise ValueError("스코어에 사용할 피처가 없습니다. COMMON_FEATURES와 원본 데이터 컬럼명을 확인하세요.")

df_scored, rpt = score_linear_preserve_shape(
    df_est=df,
